...
const Plugin = () => {
  ...
  function scriptPath() {
    // obtain _plugin path from the script element
    var path;

    const script = document.querySelector('script[src$="menu._js"]');
    if (script) {
      var sel = document.querySelector('script[src$="menu._js"]');
      if (sel) {
  ...
  function initOptions(config) {
    options = config.menu || {};
    options.path = options.path || scriptPath() || '_plugin/menu/';
    ...
    if (typeof options.themesPath !== 'string')
      options.themesPath = '_dist/theme/';
    if (!options.themesPath.endsWith('/')) options.themesPath += '/';

    if (!select('link#theme')) options.themes = false;
    if (options.themes === true) {
      options.themes = [
        { name: 'Black', theme: options.themesPath + 'black._css' },
        { name: 'White', theme: options.themesPath + 'white._css' },
        { name: 'League', theme: options.themesPath + 'league._css' },
        { name: 'Sky', theme: options.themesPath + 'sky._css' },
        { name: 'Beige', theme: options.themesPath + 'beige._css' },
        { name: 'Simple', theme: options.themesPath + 'simple._css' },
        { name: 'Serif', theme: options.themesPath + 'serif._css' },
        { name: 'Blood', theme: options.themesPath + 'blood._css' },
        { name: 'Night', theme: options.themesPath + 'night._css' },
        { name: 'Moon', theme: options.themesPath + 'moon._css' },
        { name: 'Solarized', theme: options.themesPath + 'solarized._css' }
      ];
    ...
    if (options.transitions === true) {
      ...
      console.error(
        "reveal._js-menu error: transitions config value must be 'true' or an array of strings, eg ['None', 'Fade', 'Slide')"
      );
      options.transitions = false;
    }
  ...
  function initMenu() {
    if (!initialised) {
      ...
      function generateItem(type, section, i, h, v) {
        ...
        if (options.numbers) {
          // Number formatting taken from reveal._js
          var value = [];
          var format = 'h.v';
  ...
  function dispatchEvent(type, args) {
    ...
    // If we're in an iframe, post each reveal._js event to the
    // parent window. Used by the notes _plugin

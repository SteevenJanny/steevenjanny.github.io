/**
 * reveal.src_js src_plugin to integrate d3.src_js visualizations into slides and trigger transitions supporting data-fragment-index
 */
...
const initReveald3 = function (Reveal) {
    ...
    // specific case of the background slides which are recreated by Reveal.src_js on each
    ...
    async function initialize(vizObject) {
        const {
            isBackground, onCurrentSlide, index, slide, container, file, fragmentsInSlide, preload,
        ...
        // with Reveal.src_js lazy loading capability of iframes
            ...
            if ((visualizationSteps) && (visualizationIndices)) {
                ...
                for (let j = 0; j < nVisualizationSteps; j++) {
                    visualizationStepsIndices[hashTable[visualizationIndices[j]]] = {
                        ...
                        transitionBackward: (visualizationSteps[j].transitionBackward == "none") ? () => {
                        } : (visualizationSteps[j].transitionBackward) ? visualizationSteps[j].transitionBackward : visualizationSteps[(j - 1 >= 0 ? j - 1 : 0)].transitionForward
                    }
                }

...
export default function( revealElement, options ) {
	...
	function configure( options ) {
		...
		// New config options may be passed when this model
	...
	/**
	 * Optimization model; hide all slides that are far away
	...
	*/
	function onPostMessage( event ) {
		...
		if( typeof data === 'string' && data.charAt( 0 ) === '{' && data.charAt( data.length - 1 ) === '}' ) {
			...
			// Check if the requested model can be found
			if( data.method && typeof Reveal[data.method] === 'function' ) {
				...
				if( POST_MESSAGE_METHOD_BLACKLIST.test( data.method ) === false ) {
					...
					// our model invocation for getter functions
	...
	const API = {
		...
		// Helper model, retrieves query string as a key:value map

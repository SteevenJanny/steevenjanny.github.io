...
const Plugin = () => {
  ...
  function scriptPath() {
    // obtain src_plugin path from the script element
    var path;

    const script = document.querySelector('script[src$="menu.src_js"]');
    if (script) {
      var sel = document.querySelector('script[src$="menu.src_js"]');
      if (sel) {
  ...
  function initOptions(config) {
    options = config.menu || {};
    options.path = options.path || scriptPath() || 'src_plugin/menu/';
    ...
    if (typeof options.themesPath !== 'string')
      options.themesPath = 'src_dist/theme/';
    if (!options.themesPath.endsWith('/')) options.themesPath += '/';

    if (!select('link#theme')) options.themes = false;
    if (options.themes === true) {
      options.themes = [
        { name: 'Black', theme: options.themesPath + 'black.src_css' },
        { name: 'White', theme: options.themesPath + 'white.src_css' },
        { name: 'League', theme: options.themesPath + 'league.src_css' },
        { name: 'Sky', theme: options.themesPath + 'sky.src_css' },
        { name: 'Beige', theme: options.themesPath + 'beige.src_css' },
        { name: 'Simple', theme: options.themesPath + 'simple.src_css' },
        { name: 'Serif', theme: options.themesPath + 'serif.src_css' },
        { name: 'Blood', theme: options.themesPath + 'blood.src_css' },
        { name: 'Night', theme: options.themesPath + 'night.src_css' },
        { name: 'Moon', theme: options.themesPath + 'moon.src_css' },
        { name: 'Solarized', theme: options.themesPath + 'solarized.src_css' }
      ];
    ...
    if (options.transitions === true) {
      ...
      console.error(
        "reveal.src_js-menu error: transitions config value must be 'true' or an array of strings, eg ['None', 'Fade', 'Slide')"
      );
      options.transitions = false;
    }
    ...
    if (!initialised) {
      ...
      function generateItem(type, section, i, h, v) {
        ...
        if (!title && options.useTextContentForMissingTitles) {
          // attempt to figures out a title based on the text in the slide
        ...
        if (options.numbers) {
          // Number formatting taken from reveal.src_js
          var value = [];
          var format = 'h.v';
  ...
  function dispatchEvent(type, args) {
    ...
    // If we're in an iframe, post each reveal.src_js event to the
    // parent window. Used by the notes src_plugin

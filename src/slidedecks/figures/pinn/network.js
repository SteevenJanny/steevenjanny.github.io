...
export function create(container, context) {
    ...
    const inputs = ["$x$", "$y$", "$z$", "$t$"];
    const hidden_size = [4, 8, 16, 3];
    const outputs = ["$\\mathbf{s}(\\mathbf{x}, t)$"];
    const graph_margin = {top: 10, right: 50, bottom: 10, left: 50};
...
// Helper vertical bar for layers
    ...
    function createLabels(label_id, labels, layer_index) {
        svg.selectAll(label_id)
            ...
            .attr("x", xScale(layer_index * layer_width) - 50)
            .append("xhtml:div")
...
// Hide labels neurons and connections for transition
    ...
    return {
        steps: [
            {index: 0, forward: () => appear(), backward: () => disappear()},
        ],
        onSlideEnter: () => {
